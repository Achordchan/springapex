...
function springapex_run_system_status_check(): void
{
    ...
    delete_transient('springapex_attachment_footprint_v3');
    try {
        $result = [
            'checked_at' => time(),
...
function springapex_render_system_status_page(): void
{
    ...
    <div class="wrap sa-admin sa-system-status">
      ...
      // 成本按 S3 计费口径：询盘附件里的 S3 对象 + 删除失败仍留在 S3 的孤儿对象。
      // 本地存储的旧附件不进 S3 账单，不计入。
      $billable_s3_bytes = $s3_bytes + $retry_bytes;
      $monthly_cost = ($billable_s3_bytes / GB_IN_BYTES) * 0.023; // S3 标准存储粗估
      $cost_text = $billable_s3_bytes <= 0
          ? '$0.00/月'
