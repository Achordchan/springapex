...
- REST 发布（`inc/rest-meta.php`）：产品、行业方案、案例、新闻和页面的 SEO / TDK 字段（`_springapex_seo_title`、`_springapex_seo_description`、`_springapex_seo_keywords`），以及新闻的日期文字、分类标签和相关产品（`_springapex_news_date_label`、`_springapex_news_category`、`_springapex_news_products`），可以通过 `/wp-json/wp/v2/{type}` 的 `meta` 字段读写。写入需要对该文章有 `edit_post` 权限（例如用应用程序密码认证），清洗规则与后台面板一致；这些值本来就会显示在前台，读取不额外设限。
