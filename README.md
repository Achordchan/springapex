...
- REST 发布（`inc/rest-meta.php`）：产品、行业方案、案例、新闻和页面的 SEO / TDK 字段（`_springapex_seo_title`、`_springapex_seo_description`、`_springapex_seo_keywords`），以及新闻的日期文字、相关产品和作者（`_springapex_news_date_label`、`_springapex_news_products`、`_springapex_news_author`，作者填 `/wp-json/wp/v2/spring_news_author` 里的 ID），可以通过 `/wp-json/wp/v2/{type}` 的 `meta` 字段读写。新闻分类不是 meta，用核心的 `spring_news_type` 字段传分类 ID（`/wp-json/wp/v2/spring_news_type` 可查 ID）；卡片和详情页上的小标签就是分类名。产品、行业方案、案例和新闻的正文编辑器在后台专用面板里（未声明 `editor` 支持），REST 请求中会临时补上 `editor` / `excerpt` 支持，让 `content` 和 `excerpt` 也能通过 REST 读写；后台编辑页不受影响。写入需要对该文章有 `edit_post` 权限（例如用应用程序密码认证），清洗规则与后台面板一致；这些值本来就会显示在前台，读取不额外设限。
