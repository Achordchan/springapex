...
- 新闻阅读数：卡片和详情页显示「基准数 + 真实阅读」。基准数在新闻「基础信息」里填（`_springapex_news_views_base`）；真实阅读（`_springapex_news_views`）只由访客浏览累加，后台和 REST 都不能改。计数在页面加载后由 `assets/js/news-views.js` 向 `POST /wp-json/springapex/v1/news/{id}/view` 发请求（CDN 整页缓存时渲染计数会失效）：同一浏览器同一篇 24 小时内只算一次，服务器另按 IP 10 分钟兜底并过滤爬虫，登录后台的人和预览不计。
